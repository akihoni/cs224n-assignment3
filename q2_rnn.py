...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        pad_num = max(max_length - sen_len, 0)
        if pad_num == 0:
            sen_padded = sentence[:max_length]
            label_padded = labels[:max_length]
            masking = [True] * max_length
        else:
            sen_padded = sentence + (pad_num * [zero_vector])
            label_padded = labels + (pad_num * [zero_label])
            masking = [True] * sen_len + [False] * pad_num
        ret.append((sen_padded, label_padded, masking))
        ### END YOUR CODE ###
    return ret
