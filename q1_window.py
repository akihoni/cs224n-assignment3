...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        l = 0  # 标记label位置
        # 为输入数据填充start和end标记，数量均为window_size
        sentence = [start] * window_size + sentence + [end] * window_size
        # 使用一个循环，循环产生结果list中的tuple中的list
        for i in range(window_size, sen_len + window_size):
            # 循环添加window中的特征值进目标list
            sen_list = []
            for j in range(i - window_size, i + window_size + 1):
                sen_list.extend(sentence[j])
            # 分别添加刚刚生成的list和特征值进tuple
            fin_tuple = (sen_list, labels[l])
            l += 1
            # 将生成的tuple添加进list
            windowed_data.append(fin_tuple)
    ### END YOUR CODE
